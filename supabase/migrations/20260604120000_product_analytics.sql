-- Sage 产品分析：日活埋点、管理员、仪表盘 RPC

-- ---------------------------------------------------------------------------
-- 管理员（手动插入：insert into admin_users(user_id) values ('...')）
-- ---------------------------------------------------------------------------
...
create index if not exists analytics_user_daily_date_idx
  on public.analytics_user_daily (activity_date desc);

alter table public.analytics_user_daily enable row level security;

create policy "analytics_user_daily self read" on public.analytics_user_daily
  for select using (auth.uid() = user_id);

create policy "analytics_user_daily self upsert" on public.analytics_user_daily
  for insert with check (auth.uid() = user_id);

create policy "analytics_user_daily self update" on public.analytics_user_daily
  for update using (auth.uid() = user_id) with check (auth.uid() = user_id);

-- ---------------------------------------------------------------------------
-- Helpers
-- ---------------------------------------------------------------------------
create or replace function public.analytics_today_cst()
returns date
language sql
stable
as $$
...
declare
  uid uuid := auth.uid();
  d date := analytics_today_cst();
begin
  if uid is null then
    raise exception 'unauthorized';
  end if;
  insert into analytics_user_daily (user_id, activity_date, hit_count, last_seen_at)
  values (uid, d, 1, now())
  on conflict (user_id, activity_date) do update
    set hit_count = analytics_user_daily.hit_count + 1,
        last_seen_at = now();
end;
$$;

grant execute on function public.record_analytics_activity() to authenticated;

-- 统一活跃视图（埋点 + 历史 coach_messages）
create or replace function public.analytics_user_activity_days(
  p_from date,
  p_to date
)
...
as $$
  ...
  from (
    select user_id, activity_date from analytics_user_daily
    where activity_date between p_from and p_to
    union
    select cm.user_id, (timezone('Asia/Shanghai', cm.created_at))::date
    from coach_messages cm
    where (timezone('Asia/Shanghai', cm.created_at))::date between p_from and p_to
  ) a;
$$;
...
begin
  ...
  -- Review 完成率：有 user 消息的 session → 有 review_summary
